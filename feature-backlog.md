...
> 2026-08-10 UI 节点对标专项第一批（Trigger-first / AI Agent NDV）：空白工作流的节点搜索也被约束为仅触发器，不能绕过首步；触发器首页补齐 When running evaluation 并修正 Form / Execute Workflow Trigger 的真实节点映射，On app event / Other ways 按节点子分类下钻。AI Agent 移除自身的 Anthropic 凭证与 Legacy Model/Max Tokens，参数面改为 Source for Prompt / Prompt / Specific Output / Fallback / Options，厂商凭证统一下沉到 Chat Model 子节点；仅对已经保存显式旧 `model` 参数的历史工作流保留不可见的运行期迁移兼容。NDV 中栏底部新增 Chat Model* / Memory / Tool 三个“+”，打开按 `ai_*` 输出类型过滤的节点选择器，选择后自动建立能力节点→Agent 连接。通用 `collection` 控件由 JSON 草稿升级为 Add Option 字段菜单，布尔参数改为开关+标签同排。隔离生产实例完成触发器首页、三栏 AI Agent、模型过滤与能力连线实测，控制台零 error；生产构建 6/6、全量 1148 项测试通过。

> 2026-08-10 UI 节点对标专项第二批（高频核心节点 NDV）：对照本地 n8n 实测 Manual Trigger、Schedule Trigger、Webhook、HTTP Request、Edit Fields (Set)、IF、Switch、Code 的 Input / Parameters|Settings / Output。新节点改用 Trigger Rules、Webhook URLs + HTTP Method/Respond/Options、Send Query Parameters/Headers/Body、Manual Mapping/JSON Output、Convert types/Add option、Rules/Expression、Run Once for All/Each Item 等 n8n 字段结构；通用参数控件补布尔 Fixed|Expression、条件组标题/Rename Output/数量限制、赋值类型、fixedCollection 行内 displayOptions 和代码编辑器行号/标签外观。运行时兼容旧 Schedule/Webhook/IF/Switch/Code/Set 参数，HTTP Request 补 Query/Header/Basic 凭证与二进制响应，Webhook 新建自动生成 UUID 路径。隔离生产实例已实测触发器首页及 8 类 NDV，控制台零 error；生产构建 6/6、全量 1161 项测试通过。未在本批伪装完成的差异继续记账：Webhook 短期 test-listener 生命周期、Schedule 多 Rule 持久调度、HTTP Request Import cURL、Code Python/Ask AI 实际能力。

> 2026-08-11 UI 节点对标专项第三批（第二批真实能力收口）：Webhook 新增认证态 Listen/Stop、公开 `/webhook-test/*` 一次性短期监听、超时注销、当前草稿执行和 n8n 形态 test output；Schedule 同节点每条 Trigger Rule 各自持久化 DB scheduler job，并在规则更新时停用多余 job；HTTP Request 新增 n8n 同款 Import cURL modal 与解析器，覆盖 method/URL/query/header/basic auth/JSON/form/raw body，整组导入只产生一个 undo 快照；Code 新增 Python all-items/per-item 受限子进程运行时，具备 CPU/内存/文件/超时/输出上限并封禁 import、文件、网络入口和动态执行。复核当前本地 n8n 后确认 Code 节点没有 Ask AI 控件，因此移除 Nomops 的禁用假标签。隔离实例真实验证 Webhook 首次 GET 200 回填 1 item、第二次 404，以及 cURL 参数还原、两条 Schedule Rule、Python NDV；生产构建 6/6、全量 1173 项测试通过。已记边界：多实例 test-listener 需共享 registry/sticky routing；cURL 不覆盖 multipart/client-cert/proxy 冷门 flag；Python 是安全子集而非任意包 task-runner。

> 2026-08-11 UI 节点对标专项第四批（Flow / Webhook / Form）：对照本地 n8n 实测 Merge、Loop Over Items、Wait、Execute Workflow、Respond to Webhook、Form/Form Trigger 的 Input / Parameters|Settings / Output。Merge 补齐 Append/Combine/SQL Query/Choose Branch 参数面，以及 Matching Fields/Position/All Combinations、join/output 运行时；Loop 补 Reset；Wait 补时间间隔/指定时间/Webhook/Form 四种恢复面与有限等待；Execute Workflow 补 Database/Define Below、resource locator、all-items/per-item 与等待选项；Respond to Webhook 补 8 种响应类型、状态码/Headers/Streaming 选项，并支持 all-items、二进制、JWT、redirect；Form Trigger 切换为 Authentication/Form Elements/Respond When/Options，Form 区分 Next Form Page/Form Ending，表单元素扩至 12 类并保留旧 `fields` 工作流兼容。隔离实例真实验证 trigger-first、Merge Combine、Wait Webhook 动态显隐和三栏 NDV。明确边界：Merge SQL Query 与 Execute Workflow Define Below 当前只呈现 n8n 参数面，运行时会明确拒绝而不伪执行；Form 文件上传仍受当前 JSON webhook body 边界限制。
