...
- ✅ **P3-EE2**(SSO/LDAP 表单**整页复刻**,推翻 P3-EE 的"套行式外壳保留自有字段"方案——用户裁决"完全一致的复刻";基线两页 live 控件级取证,LDAP 经点开 Enable 开关取到 18 控件全量真值):**第 17 处纠错:P3-EE 对 LDAP 的行式化方向整个错了**——基线 LDAP 是**堆叠表单**(标签在上/全宽输入/橙星必填/帮助文案在下/绿色 el-switch),非行式卡片;SSO 两表单内部布局也不同(SAML=行式 settingsItem,OIDC=堆叠 .group)。
  - **SSO/SAML**(live+源码双证):协议卡与首卡**视觉融合**(fused-top/fused-bottom,对标 protocolCard/firstCard);下拉选项逐字 **SAML|OIDC**(P3-EE 误作 "OpenID Connect");表单改元数据驱动——**Redirect URL**(=/sso/saml/callback 复制行)/**Entity ID URL**(=/sso/saml/metadata 复制行)/**Identity Provider Settings**([Metadata URL|XML] 分段 + 全宽输入),手填 IdP 三件套与 SP key 输入从 UI 移除(后端契约不变);**Role assignment** 卡("Assigned manually in nomops",单选项=真实现状);SSO Enabled 下拉带**绿点**;按钮 **Save settings + Test connection**(=新标签打开 /sso/saml/login,未启用时禁用)。
  - **SSO/OIDC**(2.31.0 源码推导,基线 OIDC 未授权无 live):堆叠 group 布局——Redirect URL(=/sso/callback 复制行)/Discovery Endpoint(↔issuer)/Client ID/Client Secret/**Prompt**(5 选项逐字)+ 卡 2 Role assignment+**ACR**+**Additional scopes**(Optional 灰字、逗号分号校验文案逐字)+ 卡 3 toggle + 双按钮。⚠ Prompt/ACR/scopes 后端未消费,本地态不发送(接线为待办)。
  - **LDAP**(live 逐字):整页堆叠复刻,字段全集 21 项按基线源码顺序——Enable LDAP Login\*(绿开关)→LDAP Login\*/Server Address\*/Port/Connection Security\*(None|TLS|STARTTLS)/[Ignore SSL/TLS Issues↰security≠none]/Base DN\*/Binding as(Admin|Anonymous)/[Binding DN/Password↰admin]/User Filter→**Attribute mapping** 节→ID\*/Login ID\*/Email\*/First Name\*/Last Name\*→sync 开关(+3 数字字段↰开)→Enforce Email Uniqueness→**[Test connection][Save connection]**(脏态互斥禁用,同基线)→**Synchronization** 节(5 列表格空态 "Test synchronization to preview updates"+双按钮)。映射:address+port+security⇄url(tls→ldaps)、Base DN⇄userSearchBase、Binding DN/Password⇄bindDn/bindPassword、Login ID/Email/First/Last⇄四属性;loginLabel/userFilter/ldapId/sync\*/enforce/anonymous 后端未消费(本地态,ledger 待办);sync 双按钮禁用(无后端)。
  - **后端 3 小件**(真实非摆设):`GET /api/sso/saml/fetch-metadata?url=`(服务端代取 IdP 元数据,CORS 不可绕;admin+saml 门控,http(s)-only+10s+1MB 限);`POST /api/ldap/test`(对已保存配置做 ldapts 服务账号 bind);samlConfigSchema+ISamlConfig 加 `idpMetadataUrl` 可选字段(仅回显)。前端 DOMParser 解析元数据 XML(命名空间无关取 EntityDescriptor/SingleSignOnService[HTTP-Redirect]/KeyDescriptor[signing] 多证书)。
  - 测试:sso-protocols.test.ts 重写为元数据流(8 项:缺省 SAML、双协议独立、锁卡、XML 双证书解析、URL 走代理、无元数据拦截、**spPrivateKey 永不回传**);frontend 57 + server **349** 全过;双端 typecheck+build 绿;live 复验 SAML/OIDC/LDAP(开关展开)三态 + 新端点 401 探活。⚠ 品牌守卫测试抓到 CSS 注释里的基线组件名,已改「基线」措辞(命名铁律)。
- ✅ **P3-EE**(5 个 EE 设置页**解锁态** UI 换成基线行式卡片体系,SSO/LDAP 经基线 saml+ldap 授权态 live 逐字取证)(⚠ SSO/LDAP 部分已被 P3-EE2 推翻重做,Log Streaming/External Secrets/Environments 三页仍以此为准):基线只授权 saml+ldap,故 **SSO/LDAP 有 live 真值**,**Log Streaming/External Secrets/Environments 基线仍锁(仅锁卡可取证),其解锁 UI 无基线可对标——按同一视觉系统做内部一致性,非逐屏复刻**。新增共享 `.set-card/.set-item/.set-item-label(320px)/.set-item-control(300px)/.set-copy/.set-field/.set-buttons`(移植基线 `sso-form.module.scss`:label 左/控件右、64px 行、分隔线、底部按钮行)。
  - **SSO**:整页重做——协议 tab→**下拉行**(Authentication protocol / Choose your SSO protocol,SAML|OpenID Connect,对标基线);OIDC/SAML 字段行式化;启用 checkbox→**Enabled/Disabled 下拉**行;SAML 新增 **SP metadata URL 复制行**(对标基线 Redirect/Entity 复制组,复制按钮 ✓ 反馈);证书用全宽 `.set-field`。⚠ **保留 nomops 真实字段不照搬基线 metadata-URL/Role assignment 模型**(后端模型不同,铁律 #5);**不加 Test connection**(nomops 无 test 端点,加了就是假控件)。`sso-protocols.test.ts` 8 项改选择器(tab→下拉 setValue、checkbox→select),7 条不变量全保。live 全量验证(OIDC 行卡 + SAML 复制行/证书块)。
  - **LDAP**:toggle-first——LDAP login Enabled/Disabled 下拉在前,连接字段(6 行)**启用后展开**(对标基线锁授权态只露 toggle+Save connection),按钮 "Save connection"。live 验证(禁用极简态 + 启用展开 6 行)。
  - **Log Streaming / External Secrets / Environments**:同体系行式化(Add destination 表单 4 行/Provider·Status·Available secrets 行/Repository URL·Branch 行 + 底部按钮)。⚠ 三页基线锁态无解锁真值,属自洽改造。live 验证三页渲染。
  - 全仓:frontend typecheck + **56 tests** + build 全绿;SSO/LDAP/三页均 nomops 侧 live 全量验证。⚠ **两 tab 卡死**(claude-in-chrome CDP `captureScreenshot` 超时/`Not attached`),换备用 tab 完成取证。
...
- 🔴 **CANVAS-HOTFIX(用户报障:节点面板行叠、折叠 rail 乱)**——两处根因均为**全局规则压制组件自身布局**:① 节点选择面板(NodePanel)的 `.node-item`/`.cat-item` 是 `<button>`,被全局 `button{height:32px}` 钉死,两行内容(标题+描述,自身 padding 12px)溢出叠行——补 `height:auto`(与 D035 tabs 同型病,第 3 次踩);② 折叠 rail:基线折叠态**不显示 logo**(仅 +/搜索/折叠开关三钮竖排),nomops 仍渲染 38×22 logo 且三钮叠进 44px 高的 brand-row——三层修复:折叠态 `display:none` 隐 logo、`flex-shrink:0` 防 flex 压缩、`height:auto` 解除全局 `.sidebar .brand-row{height:44px}`(展开态基线值)的钉死。✅ DOM 复验:rail 序列 `+16/搜索46/折叠开关76/Overview114`,无重叠无溢出;折叠开关此前被顶出屏又与 nav 重叠,现归位。📌 高危清单:全局 `button{height:32px}` 已三次压扁多行按钮(tabs/node-item/cat-item),多行按钮必须显式 `height:auto`。
- 🔴 **BORDER-ROOT-FIX(用户四度指正:表单输入框没线)**——**全仓库根源级 bug,135 处边框一直被浏览器静默丢弃**。根因链:`style.css` 把 `--border` 定义为**颜色**(`var(--border-color)`),但 vendored `design-tokens.css` 的**深色作用域**(`body[data-theme=dark]` 与 `prefers-color-scheme:dark` 媒体块,特异性高于 `:root`)把同名令牌覆盖成**完整简写** `1px solid <color>`;于是全仓库 **135 处** `border: 1px solid var(--border)` 展开为 `1px solid 1px solid <color>` —— **非法值,整条声明被丢**。表单输入框、弹窗步进器、下拉框、卡片描边在暗色主题下全部无边框,这正是用户连续截图指出的"没线"。修法:style.css(加载最晚)末尾在 `body/body[data-theme=dark]/body[data-theme=light]` 与深色媒体块内**重钉 `--border` 为颜色**,不动 vendored 文件。✅ 截图复验:Configure 弹窗的 Select 框/连体步进器(−|20|+)/Cancel 按钮边框全部复活,与用户提供的基线并排图形态一致;抽查 Overview 无"边框过多"反噬。📌 幽灵令牌家族至此第 4 起:`--panel`/`--hover`(未定义)、`--dim`(未定义)、`--border`(**被覆盖成错误类型**)——后者最隐蔽:令牌存在、值也"合理",只是**类型**不对。
- 🔴 **SETTINGS-AUDIT-3(用户三度指正:UI 画面仍不一致)**——**方法论翻案:此前所有审计都在比文本,用户看的是画面**。改用截图对比(基线/nomops 同页并排)后,壳级版式差异立现,一次修 **8 处 Settings 壳样式**(15 页全受益):容器顶距 26→**70px**(基线 `_contentContainer` 实测 70.5);页标题底距 22→**55px**;`sec-title` 18/400→**16px/600**;表头去 `uppercase`、11.5/500→**12px/600**;行高对齐 **48**(td 收 10px);开关 34×19→**42×22**(knob 13→16,位移 15→20,全局 `.switch` 一处改);设置行标题 700→**500**;Chat 页标题旁多余的 Preview 徽章移除(基线徽章只在侧栏)。✅ 截图复验:Chat 页版式与基线并排肉眼一致(表头大小写/间距/开关/行高全部归位);抽查 Users 页确认壳改无回归。📌 **铁的教训(第 20 处,也是方法级)**:文本审计通过 ≠ 视觉一致;此前 19 处纠错全靠 DOM 文本/源码,漏掉整层版式。**后续所有批次的验收标准升级为:双侧截图并排肉眼比对。**
- ✅ **SETTINGS-AUDIT-2(用户指正:操作子页/弹窗未覆盖)**——补审 Settings 全部弹窗级表面,基线 live 逐个打开取证(reka 下拉需 focus+Enter 键盘路径;全程只开不交,未写基线数据)+ 源码 `ProviderSettingsModal.vue`/i18n 对照。**修 5 处**:① **Chat Configure 弹窗**(用户点名):补 **Limit models 开关 + Allowed models 逐模型勾选**(基线 credentialId 才显 Limit,开启才显多选,空=All models)、**Default credential 清除钮**(基线 clearButton);字段 `allowedModels` 三层贯通(server settings-JSON → PATCH 校验只收本家模型 → client 类型),表格 Models 列计数源从"该家全部模型数"改为 **allowedModels**(此前语义就是错的:配了凭证就显 N models,基线含义是 Limit 生效才显);② **Change password**:inline 卡片 → 基线 **changePassword-modal 弹窗**,字段/提示/按钮逐字对齐(`New password` 提示补全 `8+ characters, at least 1 number and 1 capital letter`、`Confirm` → `Re-enter new password`、按钮 `Save password` → `Change password`);③ **Community install 弹窗**:补两处 `More info` 链接,风险句去自有尾巴对齐基线原句;④ Providers 表 Models 列语义修正(见①);⑤ Invite 弹窗经比对**逐字一致**,无需改。✅ live 复验(5173):DeepSeek 弹窗含 Clear + Limit models,开关展开列出 `deepseek-chat/deepseek-reasoner` 可勾选;表格列全显 All models(语义正确)。⚠️ **仍不一致、待裁决**:API key 弹窗日期格式(`Aug 20, 2026` vs 基线 `Aug 19 2026`)与 Custom 态 `N of N scopes selected` 句;`Use Responses API` 开关按基线仅 `provider==='openai'` 显示,nomops 无 openai 家,**留空位未实装**;2FA setup 形态(基线两步弹窗 `[1/2]`)nomops 侧未验到,待复核。
- ✅ **SETTINGS-AUDIT(用户指令:15 页全量对照基线+源码)**——双通道取证(基线 live SPA 循环采集 15 页 + 源码 i18n 逐字),nomops 侧同法采集比对。**修 8 处**(全部 live 复验 7/7 绿,MCP 空态句由代码保证):① Personal 2FA 说明去自有句、补基线 `Learn more` 链接;② Security 页补基线副标(`settings.security.description` 原句);③ OTel 补页面副标+docsLink,Enable 行由 Disabled|Enabled 下拉改为基线的**标签+开关**形态;④ MCP 空态 `Add published workflows…` → 基线原句 `Add compatible workflows…`;⑤ Chat 开关描述 `the AI Assistant is hidden` → 基线 `Chat is hidden`;⑥ Log Streaming 副标改基线 `infoText` 原句(自有 HMAC 句撤出副标);⑦ Community nodes 空态描述改基线 `empty.description.no-packages` 原句;⑧ Usage 按钮对齐基线三钮体系:`Enter activation key` 恒显、有 license 时主钮 `Manage plan`(Remove license 为自有能力保留次级位)。📌 **license 层级发现**:基线实例现为 **Business Edition**——Business 下 External Secrets/Log Streaming/自定义 Roles 仍锁,Environments/SSO/LDAP 解锁;nomops 只有单一 enterprise 层。两侧当前均为"高层级解锁态",行为相容,但**分层模型不同**(见待裁决表)。⚠️ **待裁决(功能/架构级,未动)**:Environments 表单字段集(基线 Connection Type/SSH URL/SSH Key 展示+Refresh vs nomops RepoURL/Branch,SSH key 生成需后端);Log Streaming/External Secrets 解锁态 UI 为自有实现(基线解锁形态未取证——Business 锁墙挡住,需源码逐块复刻);Chat Providers 基线全列 **15 家**(每家默认 All models 行)vs nomops 后端真实 **6 家**;License 分层(Business/Enterprise 两层 vs 单层)。
- 🔴 **HOTFIX-3(用户复报:MCP 页仍乱)**——上一轮修的是结构,这次抓到**真正的视觉炸点并 live 复验**:D142 警示条实测高 **995px**(应 ~63px),根因是 `.i13/.i14/.i15/.i16/.i18` 图标工具类**只在 OverviewView/DataTableView 的非 scoped style 里定义**,而路由懒加载 → **直达 Settings 页时这些样式尚未注入**,裸 `.i15` 的 SVG 无尺寸约束按默认尺寸炸开;先访问过 Overview 再来则正常——这解释了"时好时坏"。全仓库此类裸用 **50 处**(i13×7/i14×8/i15×20/i16×8/i18×7),同型隐患一并消除:工具类提升到**全局 style.css 常驻**。✅ **live 复验**(用户已登录的 5173 实例):警示条 995→**63px**、SVG **15×15**、Tab 行归位 y=160;Connection details 弹层实底 **460×157**、OAuth|Access token 分段 166×32、Server URL 正确指 5680。📌 教训:非 scoped 的 SFC 样式 + 懒加载路由 = "定义在别的页里"的工具类是环境依赖炸弹;工具类必须进全局表。
- 🔴 **HOTFIX-2(用户报障:侧栏展开/拖拽没了)**——**D002 判定为 ledger 严重误报(第 19 处),P1-A 据此删错了功能**。ledger 记"基线固定 201px、不可拖拽",据此移除了 nomops 的调宽把手;基线源码实为:`MainSidebar.vue` 外层是 **ResizeWrapper,min 200 / max 500**,`useSidebarLayout.onResize` 还带阈值行为(**折叠态拖过 x>100 自动展开、展开态拖到 x<100 自动折叠**),宽度存 localStorage 默认 **200**(先前量到的 201 = 200 + 1px 边框)。已恢复:store 常量改 200–500/默认 200(`setSidebarWidth` 一直都在,P1-A 只删了组件层);SideBar 加右缘把手(pointer 事件),阈值折叠/展开照抄基线;宽度由 `:style` 驱动。🔧 顺带:新增的品牌串检测测试(用户加的)抓到我注释里写了带品牌前缀的组件名,已改「基线 ResizeWrapper」表述。✅ build + typecheck + **57 项测试**全绿;⚠️ 仍无 live 渲染证据。
- 🔴 **HOTFIX(用户报障:Chat Providers 不能编辑 / MCP 页乱)**——四处修复,三处是自己批次引入的回归:① **Providers 编辑入口被 batch8 删了**:P0-batch8 做 D127 时把配置驱动的行(⋮ → Edit provider)整个换成静态 15 家镜像表;基线源码 `ChatProvidersTable.vue` + i18n `action.editProvider` 证明基线表**本就可编辑**且列的是**已配置 provider**(有 `Add provider` 与空态),静态镜像方向本身就是错的(**第 18 处纠错**)。已改回配置驱动:行 = `chatProviders`(后端真实数据),Models 列按基线 modelsText(`Disabled`/`All models`/`N models`),Last edited 用 `lastEditedAt`,行尾 ⋮ → Edit provider 接回原有 `openProviderModal`(逻辑链一直都在,只是模板断了)。② **P2-J 的 D150 只改了 thead 没改 tbody**:Connected clients 表头换成 `Client Name / Connected At / 空` 后,数据行仍是 `name/version/lastSeen` —— "Connected At" 列显示版本号、空列显示时间,整表错位。已对齐(version 并入 Name 列小字)。③ **P2-J 的 `.seg` 撞名**:新加的 `.seg/.seg-btn` 与 SSO 页既有 `.seg`(3127 行)双重定义互相覆盖,污染 SSO 分段控件;已改名 `.mcp-seg/.mcp-seg-btn`。④ **`--dim` 幽灵令牌**:5 处 `var(--dim)` 引用了不存在的令牌(同 `--panel/--hover` 一类),已统一改 `var(--text-dim)`。✅ build + typecheck + **57 项测试**全绿;⚠️ 仍无 live 渲染证据(未登录、不代输口令)。
- ✅ **P2-O**(Settings/Personal):**D145** Save 按钮补"未改动即禁用" —— 新增 `profileDirty`(与 `me` 里的原值比对),`:disabled="profileSaving || !profileDirty"`;已验边界:`saveProfile` 成功后会 `refreshMe()`,`me` 刷新后 `profileDirty` 归假,按钮正确重新禁用。⛔ **D146 判定为功能开发、非 P2,未做**:基线 Last Active 显示 `Today`,而 nomops 的 **db/server 里根本没有 `lastActive` 字段**(`grep lastActive packages/db/src packages/server/src` 为空)。要做需:加列 + 迁移 + 每请求打点 + API 暴露 + 前端相对时间格式化——与"卡片 ⋮ 补 Move"、"+ 菜单补 New variable"同属功能级待办,**不夹带进 P2 批次**,待裁决。⚠️ 同批 **D151**(Chat Configure 弹窗补 `Use Responses API` 开关)未做:需新增设置字段并贯通后端,同属功能级;**D153**(Insights 项目选择器)、**D161**(Chat 会话条目模型图标)未做:前者需项目筛选联动、后者需会话记录模型标识,均非纯样式。
- ✅ **P2-N**(画布/底部条 WorkflowCanvas/CanvasView):**D083** 点阵网格 `gap 18` → **16**(源码 `nodeViewUtils.ts` `GRID_SIZE = 16`);**D122** Chat 发送按钮由文字 `Send` 改为**纸飞机图标钮**(30×30);**D124** Logs 收起条右侧补 **popout 图标**(原仅 chevron)。❌ **D081 判定为误报(第 17 处)**:ledger 记基线控制组为 `fit/zoom-in/zoom-out/**undo(有历史后)**/Tidy up`、称 nomops "用 reset-zoom 顶替 undo"。源码 `CanvasControlButtons.vue` 实为 `zoomToFit / zoomIn / zoomOut / **resetZoom** / tidyUp` —— 第 5 个按钮**动作就是 reset zoom**,只是**图标名叫 `undo-2`**(回退箭头),ledger 把图标误读成了 undo 动作。**nomops 的 reset-zoom 本就是对的**;仅按源码补齐两处细节:图标换成 lucide `undo-2` 形状、并加上基线的条件显隐 `isResetZoomVisible = zoom !== 1`(原为恒显)。⚠️ 本批仍无 live 渲染证据;build + typecheck + **45 项测试**全绿。
- ✅ **P2-M**(NDV NdvModal/CanvasView):**D094** 头带删掉自拼的 `displayName · vN · Xms · error` 串 —— 源码 `NodeTitle.vue` 实为**只有节点图标 + 可内联改名的节点名**,别无副标;**D097** `▶ Execute step` 改用**试管图标**(源码 `useNodeExecution` 的 `buttonIcon` = `flask-conical`,由其单测 `should return flask-conical` 坐实),复用列表页已有的 flask 路径;**D120** Focus Panel 补基线提示 `Execute previous node for autocomplete`(源码 key `nodeView.focusPanel.noExecutionData` 逐字)。❌ **D090 判定为已对齐**:ledger 称 NDV Settings 用"裸 checkbox",实际 nomops 早已是 `role="switch"` 的 `.pswitch`,且尺寸 **32×16** 与基线**完全相同**。**第 16 处纠错**。🆕 **顺带发现(未做)**:基线 NDV 的节点名是 **`N8nInlineTextEdit` 可内联改名**,nomops 只是静态 span。⚠️ **D114 未做**(options 下拉改自定义带类型图标),需重写为自定义下拉(点击外关闭/键盘导航/样式),非文案级改动,单列。⚠️ 仍无 live 渲染证据;保障为 build + typecheck + **41 项测试**全绿。
- ✅ **P2-L**(凭证 5 项 CredentialModal/credential-types/OverviewView):**全部改自源码逐字真值**,不依赖运行实例。**D043** 凭证列表补分页条 —— 源码 `CredentialsView.vue` 与工作流**共用 `ResourcesListLayout`**(内含 currentPage/rowsPerPage),证实基线凭证列表确有分页;新增 `credPage/credPageSize/credTotalPages/pagedCredentials`,分页条与工作流同构(含 D038 的 100/page)。**D052** DeepSeek 的 API Key 补 `required: true` —— 源码 `DeepSeekApi.credentials.ts` 里 `apiKey` 为 required、`Base URL` 是 **hidden**(故不渲染);⚠️ ledger 另称基线有 `Allowed HTTP Request Domains / Allowed Domains` 两字段,**凭证定义里并不存在**(应为 UI 侧注入),证据不足未加。**D055** 帮助条改为基线原文 `Need help filling out these fields?` + `Read our docs`(原为自造的 `…Read the docs.`)。**D056** 类型列表由数组序改为 **`displayName.localeCompare` 字母序**(源码 `credentials.store.ts:69`)。**D057** 根因查明:命名逻辑 `` `${displayName} account` `` 两侧本就一致,**错在 displayName 多了 " API" 后缀** —— 基线 `OpenAiApi.credentials.ts` 的 `displayName = 'OpenAI'`、`DeepSeekApi` 的是 `'DeepSeek'`,已去掉后缀,默认名自然变回 `OpenAI account`。⚠️ **仍无 live 渲染证据**(Chrome 内 nomops 未登录、不代输口令):保障为 build + typecheck + 35 测试。
- ✅ **P2-K**(执行页空态/NDV 计数 CanvasView/DataPane):借 i18n `en.json` 批量确证文案。**D087** 执行页空态按基线补全:提示句改 `New workflow executions will show here`(原为自造的 `Select an execution on the left…`),并补基线的可折叠块 `Which executions is this workflow saving?` → `Production executions` / `Test executions` / `You can change this in` + `Workflow settings` 链接,两行的启用态由已有的 `savePolicy`(failed/success/manual)如实驱动,未造假数据;**D100** NDV item 计数 `{n} items` → 单复数(基线 `ndv.output.items` = `{count} item | {count} items`)。❌ **D071 / D072 判定为已对齐**:源码 `nodeCreator.triggerHelperPanel.selectATriggerDescription` = `A trigger is a step that starts your workflow`,`nodeCreator.searchBar.searchNodes` = `Search nodes...` —— nomops 现状(NodePanel.vue:112/120)与之**逐字相同**,搜索图标也已是 SVG 放大镜(非 emoji)、`›` 箭头也在。ledger 记的是旧状态,**第 14、15 处纠错**,无需改动。⚠️ **本批无 live 渲染证据**:承载 nomops 会话的预览面板 MCP 中途断开,重启的 5181/5680 在 Chrome 里是未登录态,而我不会代输任何口令(对自有 dev 实例同样适用)。保障仅为 build + typecheck + **35 项测试**全绿——比前几批弱,如实标注。
