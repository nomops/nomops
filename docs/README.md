...
> 一个节点式（node-based）可视化工作流自动化平台。对标基线，采用「共享内核 + 双部署形态」架构：先交付可自部署的 **安装版（self-hosted）**，再在其外包一层 **Cloud Web 平台**（多租户）。
...
| `16-NOMOPS-SELF-API.md` | Nomops 自 API 节点、生命周期触发器、API Key/RBAC 与固定出站边界 | 节点开发和安全审查 |
